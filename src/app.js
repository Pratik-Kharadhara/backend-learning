...
//we gonna send data to the server
const noteModel = require('./models/note.model') ; //we accepted the note model which will be used to do CRUD operations

//same as previous
app.post('/notesDB',async (req , res)=>{ // cause noteModel.create can take some time thats why its a async function
    const data = req.body;
   /*
   same as previous using req.body we get the body
    using noteModel.create and then setting the value 
    to title and description using data.title and data.description
    */
    await noteModel.create({
      title: data.title,
      description : data.description  
    }) //now the data will be sent to the DB using mongoDB

    res.status(201).json({
        message: "the data is sent succesfully"
    })
})
